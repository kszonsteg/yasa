...
from botbowl import Action, ActionType, BBDieResult, Formation, ProcBot, Skill, Square
...
class MyScriptedBot(ProcBot):
    ...
    def setup(self, game):
        ...
        for action_choice in game.get_available_actions():
            if (
                action_choice.action_type != ActionType.END_SETUP
                and action_choice.action_type != ActionType.PLACE_PLAYER
            ):
                self.setup_actions.append(Action(ActionType.END_SETUP))
    ...
    def reroll(self, game):
        ...
        if isinstance(context, botbowl.Dodge):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.Pickup):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.PassAttempt):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.Catch):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.GFI):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.BloodLust):
            return Action(ActionType.USE_REROLL)
        if isinstance(context, botbowl.Block):
            ...
            for die in context.roll.dice:
                if die.get_value() == BBDieResult.ATTACKER_DOWN:
                    ...
                    die.get_value() == BBDieResult.BOTH_DOWN
                    and not attacker.has_skill(Skill.BLOCK)
            ...
            if attackers_down > 0 and context.favor != self.my_team:
                return Action(ActionType.USE_REROLL)
            if (
                attackers_down == len(context.roll.dice)
                and context.favor != self.opp_team
            ):
                return Action(ActionType.USE_REROLL)
    ...
    def high_kick(self, game):
        ...
        if (
            game.is_team_side(game.get_ball_position(), self.my_team)
    ...
    def _make_plan(self, game: botbowl.Game, ball_carrier):
        ...
        for player in self.my_team.players:
            if (
                ...
                and not player.state.up
                and not player.state.stunned
        ...
        if (
            ...
            and ball_carrier.team == self.my_team
            and not ball_carrier.state.used
        ):
            # print("2.1 Can ball carrier score with high probability")
            td_path = pf.get_safest_path_to_endzone(
                game, ball_carrier, allow_team_reroll=True
            )
            ...
            if game.is_handoff_available():
                ...
                for player in unused_teammates:
                    ...
                    if td_path is None:
                        continue
                    handoff_path = pf.get_safest_path(
                        game, ball_carrier, player.position, allow_team_reroll=True
                    )
                    if handoff_path is None:
                        continue
                    p_catch = game.get_catch_prob(
                        player,
                        handoff=True,
                ...
                if handoff_path is not None and (
                    handoff_p >= 0.7 or self.my_team.state.turn == 8
                ):
                    self.actions.append(
            ...
            if game.num_tackle_zones_in(ball_carrier) == 0:
                ...
                for path in paths:
                    ...
                    if (
                        path.prob == 1
                        and (best_path is None or distance_to_endzone < best_distance)
                        and game.num_tackle_zones_at(ball_carrier, path.get_last_step())
                        == 0
                    ):
                        best_path = path
        ...
        if game.get_ball_carrier() is None:
            ...
            for player in self.my_team.players:
                if player.position is not None and not player.state.used:
                    if (
                        player.position.distance(game.get_ball_position())
                        <= player.get_ma() + 2
                    ):
                        path = pf.get_safest_path(
                            game, player, game.get_ball_position()
                        )
                        if path is not None:
                            p = path.prob
            ...
            if pickup_player is not None and pickup_p > 0.33:
                ...
                if (
                    game.num_tackle_zones_at(pickup_player, game.get_ball_position())
                    == 0
                    and game.get_opp_endzone_x(self.my_team)
                    != game.get_ball_position().x
                ):
                    ...
                    for path in paths:
                        ...
                        if (
                            path.prob == 1
                            and (
                                best_path is None or distance_to_endzone < best_distance
                            )
                            and game.num_tackle_zones_at(
                                pickup_player, path.get_last_step()
                            )
                            == 0
                        ):
                            best_path = path
                    ...
                    if best_path is not None:
                        self.actions.extend(
                            path_to_move_actions(
                                game, pickup_player, best_path, do_assertions=False
                            )
                        )
            ...
            if player.has_skill(Skill.CATCH) and player != ball_carrier:
                ...
                for path in paths:
                    ...
                    if (
                        path.prob == 1
                        and (best_path is None or distance_to_endzone < best_distance)
                        and game.num_tackle_zones_at(player, path.get_last_step())
                    ):
                        best_path = path
                        best_distance = distance_to_endzone
                if best_path is not None:
        ...
        if game.is_blitz_available():
            ...
            for blitzer in open_players:
                if (
                    blitzer.position is not None
                    and not blitzer.state.used
                    and blitzer.has_skill(Skill.BLOCK)
                ):
                    ...
                    for path in blitz_paths:
                        ...
                        if defender is None:
                            continue
                        from_position = (
                            path.steps[-2] if len(path.steps) > 1 else blitzer.position
                        )
                        p_self, p_opp, p_fumble_self, p_fumble_opp = (
                            game.get_blitz_probs(blitzer, from_position, defender)
                        )
            ...
            if best_blitz_attacker is not None and best_blitz_score >= 1.25:
                ...
                self.actions.extend(
                    path_to_move_actions(game, best_blitz_attacker, best_blitz_path)
                )
        ...
        if ball_carrier is not None:
            for cage_position in cage_positions:
                if game.get_player_at(
                    cage_position
                ) is None and not game.is_out_of_bounds(cage_position):
                    for player in open_players:
                        if player == ball_carrier or player.position in cage_positions:
                            continue
                        if (
                            player.position.distance(cage_position)
                            > player.num_moves_left()
                        ):
                            continue
                        if game.num_tackle_zones_in(player) > 0:
                            continue
            ...
            for opponent in game.get_adjacent_opponents(player, down=False):
                ...
                if def_str >= att_str:
                    for open_position in game.get_adjacent_squares(
                        ...
                        if (
                            len(
                                game.get_adjacent_players(
                                    open_position, team=self.opp_team, down=False
                                )
                            )
                            == 1
                        ):
        ...
        if attacker is not None and (
            p_opp_down > (1 - p_self_up) or block_p_fumble_opp > 0
        ):
            self.actions.append(Action(ActionType.START_BLOCK, player=attacker))
            self.actions.append(Action(ActionType.BLOCK, position=defender.position))
    ...
    def _get_safest_block(self, game):
        ...
        for attacker in self.my_team.players:
            if (
                ...
                and not attacker.state.used
                and attacker.state.up
            ):
                for defender in game.get_adjacent_opponents(attacker, down=False):
                    p_self, p_opp, p_fumble_self, p_fumble_opp = game.get_block_probs(
                        attacker, defender
                    )
                    p_self_up = 1 - p_self
                    if block_p_self_up is None or (
                        p_self_up > block_p_self_up and p_opp >= p_fumble_self
                    ):
    ...
    def block(self, game):
        ...
        if (
            ActionType.SELECT_BOTH_DOWN in actions
        ...
        if (
            ActionType.SELECT_BOTH_DOWN in actions
            and game.get_ball_carrier() == defender
            and not defender.has_skill(Skill.BLOCK)
        ):
            return Action(ActionType.SELECT_BOTH_DOWN)
...
def path_to_move_actions(
    game: botbowl.Game, player: botbowl.Player, path: Path, do_assertions=True
) -> List[Action]:
    """
    ...
    if do_assertions:
        if action_type is ActionType.MOVE:
            assert (
                player_at_target is None or player_at_target is game.get_active_player()
            )
    ...
    if game._is_action_allowed(final_action):
        ...
        if not player.state.up and path.steps[0] == player.position:
            ...
            actions.extend(
                Action(ActionType.MOVE, position=sq) for sq in path.steps[1:-1]
            )
        else:
            actions.extend(
