...
from yasa.yasa_core import evaluate_state_heuristic
...
def label_run(
    scores: list[float], states: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    labeled_states = []
    for score, state_dict in zip(scores, states):
        # We need the JSON string for the Rust heuristic function
        # Dumping to string is faster than reading from disk again
        state_text = json.dumps(state_dict)

        # Calculate heuristic scores
        try:
            h_heur, a_heur = evaluate_state_heuristic(state_text)
        except ValueError as e:
            print(f"Skipping state due to heuristic error: {e}")
            continue

        # Combine outcome score (score) with heuristic score
        # score is the weighted outcome for Home (+1 for Home TD, -1 for Away TD, 0 for None)

        # Home Label
        final_home = (score + h_heur) / 2.0
        final_home = max(-1.0, min(1.0, final_home))

        # Away Label
        # Outcome for away is negative of home outcome
        final_away = (-score + a_heur) / 2.0
        final_away = max(-1.0, min(1.0, final_away))

        state_dict["label_home"] = final_home
        state_dict["label_away"] = final_away

        labeled_states.append(state_dict)
    return labeled_states


def group_game_to_runs(game_dir: Path) -> list[tuple[list[dict[str, Any]], int]]:
    """
    ...
    Returns:
        List of (states_in_run, label) tuples, where states_in_run is a list of state dicts.
    """
    groups = []
    i = 0
    prev_half = None
    prev_score = None
    states_in_current_run: list[dict[str, Any]] = []

    game_file = game_dir / f"{i}.json"
    while game_file.exists():
        # Read and parse once
        state_text = game_file.read_text()
        ...
        if prev_half is None:
            ...
            states_in_current_run.append(data)
            i += 1
            game_file = game_dir / f"{i}.json"
            continue
        ...
        if half_changed:
            ...
            if states_in_current_run:
                groups.append((states_in_current_run, 0))

            # Start new run with current state
            states_in_current_run = [data]
            prev_half = current_half
            prev_score = current_score
            ...
            # Complete this run
            groups.append((states_in_current_run, label))

            # Start fresh for next run
            states_in_current_run = []
            prev_score = current_score
        else:
